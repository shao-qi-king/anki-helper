...
专为听力训练设计。查词后添加到听力牌组，Anki 正面只播放发音（不显示单词），翻面才显示单词和释义。
...
| 正面 | 背面 |
|------|------|
| 只播放发音（无文字） | 单词 + 音标 + 发音 + 中文释义 + 例句 |

复习时：听发音 → 回想单词 → 翻面确认。适合训练听音辨词能力。
...
| `LISTENING_DECK_NAME` | 听力默认牌组 | English Listening |
| `MODEL_NAME` | 单词卡片模型名 | English Vocabulary |
| `SENTENCE_MODEL_NAME` | 句型卡片模型名 | English Sentences |
| `LISTENING_MODEL_NAME` | 听力卡片模型名 | English Listening |
| `CARD_CSS` | 单词卡片样式 | — |
| `SENTENCE_CSS` | 句型卡片样式 | — |
| `LISTENING_CSS` | 听力卡片样式 | — |
